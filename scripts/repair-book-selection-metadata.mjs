...
function sanitizeSuggestedTitle(value, fallback) {
  const cleaned = String(value || '').replace(/\s+/g, ' ').trim();
  return cleaned || String(fallback || '').trim();
}

function sanitizeSuggestedAuthor(value, fallback) {
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      const res = await fetch(url, {
        headers: {
          'User-Agent': 'triggui-metadata-repair/1.0',
...
async function validateAndRepairBook(book) {
...
function buildRepairIndexes(repairs) {
  const byExact = new Map();
  const byTitle = new Map();

  for (const repair of repairs) {
    const exactKey = `${normalize(repair.input_title)}__${normalize(repair.input_author)}`;
    byExact.set(exactKey, repair);

    const titleKey = normalize(repair.input_title);
...
function findRepairForEntry(entry, indexes) {
  const title = String(entry?.title || '').trim();
  const author = String(entry?.author || '').trim();
  if (!title) return null;

  const exactKey = `${normalize(title)}__${normalize(author)}`;
  if (indexes.byExact.has(exactKey)) {
    return indexes.byExact.get(exactKey) || null;
  }

  const titleKey = normalize(title);
  const candidates = indexes.byTitle.get(titleKey) || [];
  if (candidates.length === 1) return candidates[0];

  return null;
}

function shouldApplyRepair(repair, options = {}) {
  const applyConfirmed = Boolean(options.applyConfirmed);

  if (!repair) return false;
  if (repair.status === 'normalized') return true;
  if (applyConfirmed && repair.status === 'confirmed') return true;
  return false;
}

function applyRepairToEntry(entry, repair) {
  if (!entry || !repair) return null;

  const originalTitle = String(entry.title || '').trim();
  const originalAuthor = String(entry.author || '').trim();

  const nextTitle = sanitizeSuggestedTitle(repair.suggested_title, originalTitle);
  const nextAuthor = sanitizeSuggestedAuthor(repair.suggested_author, originalAuthor);
  ...
  if (nextAuthor && nextAuthor !== originalAuthor) {
    entry.author = nextAuthor;
    changed = true;
  }
  ...
  return {
    from: { title: originalTitle, author: originalAuthor },
    to: { title: entry.title, author: entry.author },
    status: repair.status,
    rationale: repair.rationale,
  };
}

function applyRepairsToJson(json, repairs, options = {}) {
  const indexes = buildRepairIndexes(repairs);
  const appliedChanges = [];

  const applyEntry = (entry, section, index = null) => {
    const repair = findRepairForEntry(entry, indexes);
    if (!shouldApplyRepair(repair, options)) return;

    const change = applyRepairToEntry(entry, repair);
    if (!change) return;
  ...
  applyEntry(json?.selected_book, 'selected_book');

  if (Array.isArray(json?.alternatives)) {
    json.alternatives.forEach((entry, index) => applyEntry(entry, 'alternatives', index));
  }

  if (Array.isArray(json?.rejections)) {
    json.rejections.forEach((entry, index) => applyEntry(entry, 'rejections', index));
  }
...
async function main() {
  ...
  const applyConfirmed = parseBoolean(args['apply-confirmed'], false);
  ...
  const books = uniqBooks([
    json?.selected_book,
    ...(Array.isArray(json?.alternatives) ? json.alternatives : []),
    ...(Array.isArray(json?.rejections) ? json.rejections : []),
  ]);

  const repairs = [];
  for (const book of books) {
    const result = await validateAndRepairBook(book);
    repairs.push(result);
    await sleep(600);
  }
  ...
  const appliedChanges = applyRepairsToJson(json, repairs, { applyConfirmed });
  ...
  console.log(
    JSON.stringify(
      {
        output: outputPath,
        wrote_output: !dryRun && appliedChanges.length > 0,
        dry_run: dryRun,
