...
const DEFAULTS = {
  ...
  comparisonPrefix: 'book-selection__case-003__comparison__new-vs-legacy',
};
...
function extractSelectedBook(run) {
  ...
  if (Array.isArray(run?.recommended_books) && run.recommended_books.length > 0) {
    const first = run.recommended_books[0];
    return {
...
function extractAlternatives(run) {
  ...
  if (Array.isArray(run?.recommended_books) && run.recommended_books.length > 1) {
    return run.recommended_books.slice(1).map((item) => ({
      title: pickTitle(item),
      author: pickAuthor(item),
      reason: pickReason(item),
    }));
  ...
  if (Array.isArray(node)) {
    const looksLikeBookArray = node.some((item) => {
      ...
      if (item && typeof item === 'object') {
        return Boolean(
          item.title || item.titulo || item.book_title || item.nombre || item.name
        );
      }
      return false;
    });
...
async function main() {
  ...
  const comparisonPath = await getNextComparisonPath(
    String(args.outputPrefix || DEFAULTS.comparisonPrefix)
  );
  ...
  const report = `# Comparación formal — case-003 — new-system vs legacy-baseline
...
- \`selection_source === "catalog"\`:
  ...
  - Legacy baseline: ${
    legacySelected.selection_source === null ? 'No determinable (schema legacy)' : yesNo(legacySelected.selection_source === 'catalog')
  }
